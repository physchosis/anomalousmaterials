(function(angular) {
    ...
    CardsController.prototype.processMarkdownType = function(self) {
            ...
            self.$http({method:'GET', url: fileURL}).then(function successCallback(newResp) {
                 var md = window.markdownit();
...
;
    ...
    CardsController.prototype.maximize = function(cardID) {
        ...
        $(cardElm).css({'position':'absolute','z-index':'1000','top': cardTop, 'left':'0px', 'margin':'0px','width': windowWidth, 'height':windowHeight});
        $(cardElm).height(windowHeight-navBarCurHeight-5+'px');
        $(cardElm).width(document.body.scrollWidth-5+'px');
        $(contentElm).css({'overflow-y':'scroll', 'max-height':windowHeight-navBarCurHeight-cardHeaderHeight-75+'px'});
        ...
        $(cardSource).css({'overflow-y':'scroll', 'max-height':windowHeight-navBarCurHeight-cardHeaderHeight-75+'px'});
    ...
    CardsController.prototype.minimize = function(cardID) {
        ...
        $(cardElm).css({'position':'relative', 'width': '100%', 'height':'100%'});
        $(contentElm).removeAttr('style');
        $(contentElm).css({'overflow-y':'scroll', 'max-height':'200px'});
        $(cardSource).removeAttr('style');
        $(cardSource).css({'overflow-y':'scroll', 'max-height':'200px'});
        // reset view
        this.showCard = true;
        this.showAmpSource = false;
